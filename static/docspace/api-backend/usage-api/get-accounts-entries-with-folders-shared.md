...
| **employeeTypes** | query | [**List**](../people.md#model-employeetype) | Keeps only the users of the listed types, combined as alternatives. An empty list, which is the default,  searches every type. | [optional] [example: [RoomAdmin, Guest]] |
...
|------------- | ------------- | ------------- | -------------|
| **200** | The matching users and groups, each with its access state for the folder | [**IAccountEntryArrayWrapper**](../people.md#model-iaccountentryarraywrapper) | `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset` |
